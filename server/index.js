...
function initializeApp() {
  ...
  app.use((req, res, next) => {
    const timeout = serverConfig.http.timeout || 30000;
  ...
  app.use(express.json({ 
    limit: '1mb', // Reduced from 10mb to prevent DoS
    verify: (req, res, buf) => {
  ...
  app.get('/health', async (req, res) => {
    ...
    try {
      ...
      if (result.rows && result.rows[0] && result.rows[0].health_check === 1) {
        dbStatus = 'connected';
      } else {
        dbStatus = 'error';
  ...
  if (!serverConfig.env.isProduction || process.env.ENABLE_SWAGGER === 'true') {
  ...
  app.use('/demo', demoRoutes); // Demo mode endpoints
